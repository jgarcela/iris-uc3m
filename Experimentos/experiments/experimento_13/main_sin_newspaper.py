...
def procesar_fila(row):
    ...
    # 25. Lenguaje Sexista
    lenguaje_sexista = variables.clasificar_var_lenguaje_sexista(texto, ruta_json=RUTA_VARIABLES_JSON, ruta_template=RUTA_TEMPLATE, modelo='gemma4:e4b')
    resultados.update(_expandir_resultado(lenguaje_sexista, 'modelo_lenguaje_sexista'))

    # 26. Masc Generico
    masc_generico = variables.clasificar_var_masc_generico(texto, ruta_json=RUTA_VARIABLES_JSON, ruta_template=RUTA_TEMPLATE, modelo='gemma4:e4b')
    resultados.update(_expandir_resultado(masc_generico, 'modelo_masc_generico'))

    # 30. Sexismo Discurso
    sexismo_discurso = variables.clasificar_var_sexismo_discurso(texto, ruta_json=RUTA_VARIABLES_JSON, ruta_template=RUTA_TEMPLATE, modelo='gemma4:e4b')
    resultados.update(_expandir_resultado(sexismo_discurso, 'modelo_sexismo_discurso'))

    # 33. Asimetria Mujer Hombre
    asimetria_mujer_hombre = variables.clasificar_var_asimetria_mujer_hombre(texto, ruta_json=RUTA_VARIABLES_JSON, ruta_template=RUTA_TEMPLATE, modelo='gemma4:e4b')
    resultados.update(_expandir_resultado(asimetria_mujer_hombre, 'modelo_asimetria_mujer_hombre'))

    # 35. Denominacion Sexualizada
    denominacion_sexualizada = variables.clasificar_var_denominacion_sexualizada(texto, ruta_json=RUTA_VARIABLES_JSON, ruta_template=RUTA_TEMPLATE, modelo='gemma4:e4b')
