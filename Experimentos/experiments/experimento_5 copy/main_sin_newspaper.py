...
# CONFIGURACIÓN vLLM
# ==========================================
# TITAN Xp (compute capability 6.1): solo es compatible con GPTQ (vía ExLlama, CC >= 6.0)
# AWQ y bfloat16 requieren CC >= 7.5 y 8.0 respectivamente
MODEL_VLLM = "Qwen/Qwen2.5-7B-Instruct-GPTQ-Int4"

print(f"Cargando modelo {MODEL_VLLM} con vLLM en GPU...")
_llm = LLM(
    model=MODEL_VLLM,
    quantization="gptq",
    dtype="float16",
...
def _consultar_vllm(prompt: str, modelo: str = MODEL_VLLM, temperature: float = 0.1) -> str:
    """
    Reemplaza consultar_ollama usando vLLM directamente en GPU.
...
FOLDER = "../../results"
PREFFIX = "5-Experimento-5_03_2026_gpu"
SUFFIX = "scrape"
