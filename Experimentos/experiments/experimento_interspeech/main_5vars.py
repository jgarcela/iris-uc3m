...
for modelo_actual in MODELOS:
    for prompt_actual in lista_prompts:
        ...
        # AQUÍ ESTÁ EL CAMBIO: Añadimos 'desc' a tqdm para que muestre el modelo y prompt actual
        texto_tqdm = f"{modelo_actual} | {prompt_limpio}"
        for index, row in tqdm(df_procesar.iterrows(), total=df_procesar.shape[0], desc=texto_tqdm):
        ...
        # (Esto va justo después de terminar el bucle for index, row in tqdm(...): )
        print(f"\n✅ Extracción finalizada para {nombre_exp}.")

        # =======================================================
        # 🛡️ FILTRO DE SEGURIDAD PARA NO ROMPER LAS MÉTRICAS
        # =======================================================
        print("🧹 Limpiando errores antes de calcular métricas...")
        
        # Leemos el CSV completo que acabamos de generar (que contiene los textos "ERROR")
        df_completo = pd.read_csv(nombre_output)
        
        # Filtramos quedándonos SOLO con las filas donde el modelo NO falló
        df_limpio = df_completo[df_completo['modelo_fallo'] == False].copy()
        
        # Guardamos esta versión limpia en un archivo temporal para las métricas
        nombre_output_limpio = nombre_output.replace(".csv", "_limpio.csv")
        df_limpio.to_csv(nombre_output_limpio, index=False)
        
        # Así, en tu archivo *_resultados_2024_scrape.csv tienes TODO (incluidos los errores para que los revises).
        # Y en tu archivo *_limpio.csv tienes solo lo correcto para que las métricas funcionen perfectas.
        # =======================================================

        # 4.1 GENERAR MÉTRICAS GENERALES (¡Le pasamos el archivo limpio!)
        generar_metricas_y_summary(
            archivo_input=nombre_output_limpio,  # <-- CAMBIO AQUÍ
            archivo_output=archivo_metricas,
            nombre_experimento=nombre_exp,
            modelo_usado=modelo_actual,
            prompt_file=prompt_actual
        )

        # 4.2 GENERAR MATRICES DE CONFUSIÓN (¡Le pasamos el archivo limpio!)
        print("📊 Generando matrices de confusión...")
        guardar_matrices_confusion(nombre_output_limpio, nombre_exp, FOLDER_MATRICES)  # <-- CAMBIO AQUÍ
