...
\subsection{Resultados por variable: el efecto del empaquetado}
\label{subsec:iris_porvariable_b0b1}
...
\input{chapters/5 results and discussion/tablas_porvariable}

El primer resultado es que \textbf{el efecto de las \textit{skills} no es uniforme
ni entre modelos ni entre variables}. En \texttt{masc\_generico} (V26)
\texttt{gemini-3.1-flash-lite} mejora de forma clara al pasar a B1, ya que su
recall sube de 0,287 a 0,733 y su F1 de la clase positiva de 0,435 a 0,803, esto
es, pasa de detectar poco más de la cuarta parte del masculino genérico que marcan
las expertas a detectar casi tres cuartas partes. La exactitud acompaña (de 0,397 a
0,708) y también lo hace el $\kappa$ (de 0,073 a 0,261). Es la única celda del
capítulo en que todas las métricas se mueven a la vez en la dirección esperada, lo
que descarta que la mejora responda a un artefacto de la distribución. En
\texttt{asimetria\_mujer\_hombre} (V33) el modelo local mejora también con B1, con
un recall que pasa de 0,136 a 0,160 y un F1 de 0,120 a 0,151.

En sentido contrario, las \textit{skills} degradan varias celdas en las que el
control ya tenía algo de señal, y lo hacen sobre todo en
\texttt{denominacion\_sexualizada} (V35). Ahí el recall de \texttt{gemma4:e4b} cae
de 0,130 a 0,023 y el de \texttt{gpt-4o-mini} de 0,107 a 0,046, con descensos
paralelos en F1 y en $\kappa$ (este último de 0,154 a 0,015 y de 0,136 a 0,061
respectivamente).

El caso de \texttt{lenguaje\_sexista} (V25) en \texttt{gpt-4o-mini} ilustra por qué
conviene mirar varias métricas a la vez. Al pasar a B1 su exactitud casi se duplica
...
desacoplamiento, que reaparece en varias celdas, se examina como cuestión
metodológica en la Sección~\ref{subsec:iris_divergencia}.
...
Establecido el efecto del empaquetado, procede caracterizar cómo se comporta el
sistema desplegado, esto es, el nivel B1, en cada una de las cinco variables.

\subsubsection{Comportamiento por variable en el nivel B1}

El comportamiento de cada variable se examina a continuación sobre las
Tablas~\ref{tab:var-v25} a~\ref{tab:var-v35} ya presentadas, atendiendo en cada caso
a la métrica que resulta informativa según su prevalencia
(Sección~\ref{sec:metricas_eval}). El recall merece atención particular en todas
ellas, ya que indica qué proporción del sexismo señalado por las expertas llega a
detectar el sistema, y es la magnitud que decide su utilidad como herramienta de
apoyo.
...
\begin{itemize}
    ...
    (Tabla~\ref{tab:iris-equipos-recall}), es decir, coinciden en que el fenómeno está
    ...
    Merece una nota el \emph{salto semántico}, el caso en que un masculino
    presentado como genérico se revela no inclusivo al aparecer después una marca
    explícita de mujer, que el libro de códigos distingue con el valor~3 de esta
    variable (Sección~\ref{sec:iris_variables}). El sistema lo contempla y llega a
    emitirlo, 37 veces \texttt{gpt-4o-mini} en el nivel B1 y de forma testimonial
    los demás modelos. La anotación experta de este corpus, en cambio, no emplea
    ese valor en ninguna de las 1.313 piezas, que se reparten entre 250 negativos y
    1.063 positivos ordinarios. No es posible, por tanto, evaluar si el sistema
    detecta correctamente el salto semántico, y esa es también la razón por la que
    la agrupación binaria de los valores 2 y 3 no altera en nada la referencia. La
    identificación de este fenómeno queda como línea de trabajo futura, condicionada
    a que una anotación posterior lo codifique de forma diferenciada.

    \item \textbf{\texttt{masc\_generico} (V26): el único acuerdo genuino, y su
    umbral.} Es la única variable en que el sistema detecta de verdad.
    \texttt{gemini} recupera el 73,3\,\% del masculino genérico que marcan las
    expertas, con una precisión de 0,887 y un F1 de 0,803, cifras que lo convierten
    en un detector utilizable. Todas las métricas mejoran a la vez sobre el
    \textit{baseline}, incluido el $\kappa$ (0,261), lo que descarta el artefacto de
    marginales y confirma una ganancia real. El local \texttt{gemma4:e4b} queda en
    segundo lugar, con un recall de 0,408 y un F1 de 0,561. Los dos modelos de OpenAI, en cambio, son mucho más
    ...
    motivo.} El recall se mueve entre 0,020 y 0,114, de manera que, pese a una
    prevalencia anotada del 42,8\,\%, los modelos dejan sin señalar más del 88\,\% de
    los casos, y el F1 de la clase positiva no supera 0,181. El acuerdo ronda además
    el azar en los cuatro ($\kappa$ entre $-0,006$ y 0,011). La
    ...
    Tabla~\ref{tab:iris-equipos-recall}), mientras que en V25 ambos equipos coincidían en
...
\subsubsection{Un caso concreto}
