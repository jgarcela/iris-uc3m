...
contrastarlo (Sección~\ref{subsec:limitacion_fiabilidad}). Las cifras que siguen
miden si el sistema clasifica como las expertas, no si señala los mismos fragmentos
ni si sus justificaciones resultan aceptables.
...
cada variable, ya expuesta en la Sección~\ref{sec:iris_variables}. Se reproduce
aquí por comodidad, dada su importancia para interpretar lo que sigue. La
Tabla~\ref{tab:iris-referencia} recoge la prevalencia de la clase positiva y el
rendimiento del clasificador trivial que marca todas las piezas, cifras que
constituyen el umbral mínimo que un sistema debe superar para aportar información.
...
\begin{table}[H]
    ...
    \small
    \ttabbox[\FBwidth]{
        \caption{Prevalencia anotada y rendimiento del clasificador trivial que marca
        todas las piezas como positivas ($N=1.313$), reproducida de la
        Sección~\ref{sec:iris_variables}.}
        \label{tab:iris-referencia}
    }{
        \begin{tabular}{@{}l l c >{\columncolor{gray!15}}c >{\columncolor{gray!15}}c@{}}
            \toprule
            \textbf{Cód.} & \textbf{Variable} & \textbf{Prevalencia} & \textbf{Exactitud} & \textbf{F1 (\textit{Sí})} \\
            \midrule
            V25 & \texttt{lenguaje\_sexista}         & 0,810 & 0,810 & 0,895 \\
            V26 & \texttt{masc\_generico}            & 0,810 & 0,810 & 0,895 \\
            V30 & \texttt{sexismo\_discurso}         & 0,428 & 0,428 & 0,599 \\
            V33 & \texttt{asimetria\_mujer\_hombre}  & 0,062 & 0,062 & 0,117 \\
            V35 & \texttt{denominacion\_sexualizada} & 0,100 & 0,100 & 0,182 \\
            \bottomrule
        \end{tabular}
    }
    \endgroup
