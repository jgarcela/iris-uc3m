...
La primera se deriva del reparto entre los dos límites
(Sección~\ref{subsec:iris_dos_limites}). \texttt{sexismo\_discurso}, la variable en la
que el criterio experto no llega a estabilizarse, no admite seguir siendo tratada como
si tuviera una única interpretación correcta. Resultaría prometedor explorar
...
aquí solo acotado de forma indirecta.
...
local, competitivo variable a variable y sin coste de tarifas
...
un análisis de contenido experto necesita. Sobre los controles de aceptación, edición y
rechazo que la herramienta ya ofrece (Sección~\ref{poc}), cabe construir un ciclo de
retroalimentación en el que esas decisiones sirvan para reajustar los criterios y los
umbrales por variable, de forma coherente con el papel de asistente, y no de decisor,
que se ha argumentado para el sistema.

Por último, en el plano de la aplicación, el sistema se ha probado sobre la
herramienta web del proyecto IRIS (Sección~\ref{poc}), y el paso siguiente es validar
esa prueba con estudios de uso con profesionales de redacciones reales, que son
los que permitirán saber si analizar el corpus y resaltar los pasajes candidatos alivia
efectivamente el trabajo de las analistas. Un
escenario de aplicación natural es el apoyo a los flujos de revisión editorial de un
medio de comunicación, y en particular de una corporación de radiotelevisión pública,
donde el seguimiento de la representación de las mujeres forma parte de su mandato de
servicio público. Dado que este trabajo se ha desarrollado con el apoyo de la ayuda
Impulsa Visión de RTVE (Sección~\ref{sec:research_context}), un paso futuro natural es
colaborar con la corporación para llevar el sistema a un entorno de redacción real. Esa
