%----------
%	LLMs, AGENTES Y AGENT SKILLS PARA LA DETECCIÓN DE SESGO
%----------

Las herramientas descritas en la sección anterior comparten un límite: operan
sobre reglas fijas, léxicos cerrados o clasificadores entrenados para una
etiqueta concreta. La irrupción de los grandes modelos de lenguaje (LLM) y, sobre
ellos, de las arquitecturas de agentes ha abierto una vía distinta, en la que el
conocimiento experto ya no se codifica en un diccionario sino que se comunica al
modelo en lenguaje natural. Esta sección revisa esa frontera, que es la que
...
marcado, como el español, donde el fenómeno es más sutil que en inglés. El trabajo
de Derner et~al.~\cite{derner2024gender} es una de las primeras propuestas de usar
LLMs para identificar y clasificar el sesgo de representación de género en corpus
...
Un \textit{agente} basado en LLM va más allá de una única llamada al modelo: combina
...
agentes especializados se reparten una tarea compleja. En el ámbito periodístico,
AI-Press~\cite{liu2025aipress} asigna agentes a roles editoriales (recuperación,
redacción, revisión) apoyados en recuperación aumentada. Ahora bien, la
introducción de estos sistemas en las redacciones reactiva una exigencia clásica
del periodismo algorítmico: la rendición de cuentas y la trazabilidad, es decir,
que ningún sistema opere como una caja negra y que toda afirmación pueda rastrearse
hasta su fuente~\cite{diakopoulos2019}. Ese requisito es precisamente uno de los
que guía el diseño del sistema propuesto en este trabajo.
...
para dotar a un agente de conocimiento especializado~\cite{anthropic2025skills}.
...
progresiva} (\textit{progressive disclosure}): el agente solo ve el nombre y la
...
ámbito de los agentes: procede del diseño de interacción persona-máquina (Human-Computer Interaction, HCI en inglés), donde se
...
\textit{Agent Skills} le da una formulación operativa para modelos de lenguaje. La observación empírica que
motiva este enfoque es que, en muchas tareas, una instrucción profesional bien
redactada supera a infraestructura más sofisticada como un sistema de recuperación
vectorial. Este resultado, contraintuitivo, sitúa el conocimiento experto en el centro del diseño, y es la idea sobre la que se
articula la arquitectura del Capítulo~\ref{implementation}.

\subsection{La intersección: agentes para detectar sesgo}

La combinación de agentes y detección de sesgo es la frontera más reciente y donde
se sitúa este trabajo. Bias-Aware Agent~\cite{singh2025biasaware} incorpora la
detección de sesgo como una \textit{herramienta} del agente en lugar de como un
...
separa hecho de opinión antes de puntuar la intensidad del sesgo, y Huang y
Somasundaram~\cite{huang2024queer} muestran que una arquitectura de agentes
especializados mejora de forma notable a un único modelo generalista en una tarea
...
disparidades sobre datos parlamentarios europeos. Con todo, ninguno de estos
trabajos aborda la detección de lenguaje sexista en prensa escrita en español
mediante agentes especializados que encapsulen la metodología experta como
\textit{skills}: ese es el hueco que este Trabajo de Fin de Máster pretende cubrir.
