...
Concluido el trabajo, el balance es el siguiente. Cuatro de los cinco riesgos se
materializaron, uno de ellos solo de forma parcial, y en ellos las medidas previstas
resultaron aplicables, aunque no siempre suficientes. El quinto no llegó a necesitarse.
...
sensibilidad por equipo de anotación permitió acotar el techo de acuerdo de la tarea en
\texttt{sexismo\_discurso} y, a la vez, mostrar una infradetección de los modelos en
variables donde las anotadoras coinciden (Sección~\ref{subsec:iris_errores}).
...
(Sección~\ref{genai_models}).

El \textbf{R3} se materializó y su análisis es una de las aportaciones del trabajo. La
recuperación aumentada (RAG), prevista como medida de mitigación, se implementó y se
evaluó de forma aislada, con el resultado de que no aporta una ganancia que justifique su coste
...
en la herramienta web del proyecto IRIS (Sección~\ref{poc}) y las directrices de uso
...
solo (Sección~\ref{genai_models}). Por otro, las ejecuciones sobre la granja
de unidades de procesamiento gráfico (\textit{Graphics Processing Unit}, GPU por sus
siglas en inglés) sufrieron interrupciones, para lo que se implementó una ejecución reanudable y
