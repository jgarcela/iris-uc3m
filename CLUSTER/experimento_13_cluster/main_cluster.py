"""
Experimento 13 adaptado para el cluster TSC (fase CLIENTE).

Igual que el cliente del experimento_12, pero aplicando las 5 variables de SEXISMO
del experimento_13 (lenguaje sexista, masc genérico, sexismo discurso, asimetría
mujer/hombre, denominación sexualizada). Cada variable devuelve código +
explicación + evidencias (columnas 'modelo_<var>', '_explicacion', '_evidencias').

Ataca a un servidor Ollama levantado por launch_process.py (llm.json). Se lanza
una vez por servidor/GPU, repartiendo el dataset en shards.

Ejemplo (un cliente contra un servidor):
    OLLAMA_HOST=bastet07:11434 \
    python main_cluster.py --shard 0 --n-shards 2 --workers 4 \
...
# ==========================================
def parse_args():
    p = argparse.ArgumentParser(description="Experimento 13 - cliente cluster (sharded)")
...
# Ollama devuelve 'total_duration' (ns) por petición. Envolvemos ollama.chat para
# acumularlo por hilo y medir el cómputo del modelo por artículo aunque haya
# concurrencia (--workers > 1).
...
PREFFIX = "13-Experimento-13_03_2026"
...
# ==========================================
# 3. FUNCIÓN DE PROCESAMIENTO (5 variables de sexismo del exp13)
# ==========================================
